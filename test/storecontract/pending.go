...
func testPendingQuestion(t *testing.T, f Fixture) {
	...
	found, err := repo.Get(ctx, "pq-1")
	require.NoError(t, err)
	require.NotNil(t, found)
	assert.Equal(t, q.QuestionsJSON, found.QuestionsJSON)
	assert.Equal(t, "card-1", found.CardID)

	pending, err := repo.ListByConversationAndStatus(ctx, "conv-1", store.PendingQuestionStatusPending)
	require.NoError(t, err)
	...
	require.NoError(t, repo.SetStatus(ctx, "pq-1", store.PendingQuestionStatusAnswered))
	found, err = repo.Get(ctx, "pq-1")
	require.NoError(t, err)
	require.NotNil(t, found)
	assert.Equal(t, store.PendingQuestionStatusAnswered, found.Status)
	assert.Equal(t, q.QuestionsJSON, found.QuestionsJSON)
